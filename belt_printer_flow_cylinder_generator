...
class GCodeGenerator:
    ...
    def generate_gcode(self):
        # Keep the base gcode exactly as is, but remove trailing whitespace
        base_gcode_cleaned = self.base_gcode.rstrip()
        ...
        continuation_gcode += f"; Generated on: 2025-03-04 23:09:53\n"
        ...
        continuation_gcode += f"M104 S{self.nozzle_temp} ; Set nozzle temperature\n"
        continuation_gcode += f"M140 S{self.bed_temp} ; Set bed temperature\n"
...
def main():
    try:
        root = tk.Tk()
        root.title("Belt Printer G-Code Generator")
